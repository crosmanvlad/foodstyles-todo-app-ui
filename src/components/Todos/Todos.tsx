    ...
    <div className="todos-container">
      <div className="todos-body-container">
        ...
        <TextField
            ...
            />
        <div>
          {filteredTodos.sort((a,b) => a.id > b.id ? 1:-1).map((todo: any) => (
            <div key={todo.id} className="todo-item">
              <div className="todo-item-left-wrapper">
                <Checkbox checked={todo.status} onChange={() => handleUpdate(todo)} />
                <div>{todo.title}</div>
              </div>
              <IconButton color="default" aria-label="delete todo" component="label" onClick={() => handleDelete(todo)}>
                <MdClose />
              </IconButton>
            </div>
          ))}
